...
if (root == nullptr || root->data == x) // If the current node is null or contains the target
value 'x'
...
root->right_child = insert(root->right_child, x); // Recursively insert 'x' into the right
subtree
else // If 'x' is smaller than the current node's data
...
return find_minimum(root->left_child); // Recursively find the minimum value in the left
subtree
return root; // Return the current node
}
// Deletion
...
root->right_child = delete_node(root->right_child, x); // Recursively delete 'x' from the
right subtree
else if (x < root->data) // If 'x' is smaller than the current node's data
root->left_child = delete_node(root->left_child, x); // Recursively delete 'x' from the left
subtree
...
Node* temp = find_minimum(root->right_child); // Node has two children, find the
minimum value in the right subtree
root->data = temp->data; // Copy the data from the minimum node to the current node
root->right_child = delete_node(root->right_child, temp->data); // Recursively delete the
minimum node from the right subtree
}
}
return root; // Return the current node
}
